...
function tex = mt_generate_student_report(student_data, ...
                                          questions_title, ...
                                          questions_remarks, ...
                                          final_mark, ...
                                          cw_settings)
...
%% Read template
tex = fileread(cw_settings.feedback_template_filename);

%% Change template title
tex = strrep(tex, 'MT_DOCUMENT_TITLE', ...
                            cw_settings.feedback_title);
...
for i = 1:num_questions
    if strlength(questions_remarks{i}) == 0
        questions_remarks{i} = cw_settings.message_no_remarks;
    end
